<div class="table-responsive-sm">
    <table class="table table-striped" id="announcements-table">
        <thead>
            <tr>
                <th>{{__("validation.attributes.ImageUrl")}}</th>
        <th>{{__("validation.attributes.TitleRu")}}</th>
        <th>{{__("validation.attributes.TitleKz")}}</th>
        <th>{{__("validation.attributes.TitleEn")}}</th>
        <th>{{__("validation.attributes.SubtitleRu")}}</th>
        <th>{{__("validation.attributes.SubtitleKz")}}</th>
        <th>{{__("validation.attributes.SubtitleEn")}}</th>
        <th>{{__("validation.attributes.Link")}}</th>
        <th>{{__("validation.attributes.Status")}}</th>
        <th>{{__("validation.attributes.PublishedAt")}}</th>
                <th colspan="3">
                    {{__("validation.attributes.Action")}}
                </th>
            </tr>
        </thead>
        <tbody>
        @foreach($announcements as $announcement)
            <tr>
                <td><img src="{{ $announcement->getFile("imageUrl") }}" style="max-width: 100px"></td>
            <td>{{ $announcement->titleRu }}</td>
            <td>{{ $announcement->titleKz }}</td>
            <td>{{ $announcement->titleEn }}</td>
            <td>{{ $announcement->subtitleRu }}</td>
            <td>{{ $announcement->subtitleKz }}</td>
            <td>{{ $announcement->subtitleEn }}</td>
            <td>{{ $announcement->link }}</td>
            <td>{{ $announcement->status == true ? __("validation.attributes.Active") :__("validation.attributes.Non-Active")  }}</td>
            <td>{{ $announcement->publishedAt }}</td>
                <td>
                    {!! Form::open(['route' => ['announcements.destroy', $announcement->id], 'method' => 'delete']) !!}
                    <div class='btn-group'>
                        <a href="{{ route('announcements.show', [$announcement->id]) }}" class='btn btn-ghost-success'><i class="fa fa-eye"></i></a>
                        <a href="{{ route('announcements.edit', [$announcement->id]) }}" class='btn btn-ghost-info'><i class="fa fa-edit"></i></a>
                        {!! Form::button('<i class="fa fa-trash"></i>', ['type' => 'submit', 'class' => 'btn btn-ghost-danger', 'onclick' => "return confirm(".__("validation.attributes.QuestionDelete").")"]) !!}
                    </div>
                    {!! Form::close() !!}
                </td>
            </tr>
        @endforeach
        </tbody>
    </table>
</div>
