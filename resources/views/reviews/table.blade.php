<div class="table-responsive-sm">
    <table class="table table-striped" id="reviews-table">
        <thead>
            <tr>
                <th>{{__("validation.attributes.ImageUrl")}}</th>
        <th>{{__("validation.attributes.Name")}}</th>
        <th>{{__("validation.attributes.Comment")}}</th>
                <th colspan="3">{{__("validation.attributes.Action")}}</th>
            </tr>
        </thead>
        <tbody>
        @foreach($reviews as $review)
            <tr>
                <td><img src="{{ $review->getFile("imageUrl") }}" style="max-width: 120px"></td>
            <td>{{ $review->name }}</td>
            <td>{{ $review->comment }}</td>
                <td>
                    {!! Form::open(['route' => ['reviews.destroy', $review->id], 'method' => 'delete']) !!}
                    <div class='btn-group'>
                        <a href="{{ route('reviews.show', [$review->id]) }}" class='btn btn-ghost-success'><i class="fa fa-eye"></i></a>
                        <a href="{{ route('reviews.edit', [$review->id]) }}" class='btn btn-ghost-info'><i class="fa fa-edit"></i></a>
                        {!! Form::button('<i class="fa fa-trash"></i>', ['type' => 'submit', 'class' => 'btn btn-ghost-danger', 'onclick' => "return confirm(".__("validation.attributes.QuestionDelete").")"]) !!}
                    </div>
                    {!! Form::close() !!}
                </td>
            </tr>
        @endforeach
        </tbody>
    </table>
</div>
